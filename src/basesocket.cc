...
tcp::basic_socket::basic_socket() : host(NULL), port(0) {
  socketfd = socket(AF_INET, SOCK_STREAM, 0); // creates the new socket
  if (socketfd < 0) {
    // cerr << "basesocket.cc:17:25: Error creating socket descriptor (non-zero
    // return)" << endl;
    // set errno
...
tcp::basic_socket::basic_socket(const char *_host, int _port)
    : host((char *)_host), port(_port) {
  socketfd = socket(AF_INET, SOCK_STREAM, 0); // creates the new socket
  if (socketfd < 0) {
    // cerr << "basesocket.cc:31:44: Error creating socket descriptor (non-zero
    // return)" << endl;
    throw new ctor_exe_t(); // throw constructor exception
  }
}
...
tcp::basic_socket::basic_socket(int sockd) : socketfd(sockd) {}

int tcp::basic_socket::getSockfd() { return this->socketfd; }

bool tcp::basic_socket::isConnected() {
  int error = 0;
  ...
  return (ret == 0 && error == 0)
             ? true
             : false; // if both are true, then socket is connected
}

int tcp::basic_socket::connects(const char *_host, int _port) {

  server_data = gethostbyname(_host); // set server data
  if (server_data == NULL) {
    cerr << "basesocket.cc:connects:66: Error encountered in gethostbyname -- "
            "NO SUCH HOST"
         << endl;
    return 1;
  }
  ...
  bcopy((char *)server_data->h_addr, (char *)&server_address.sin_addr.s_addr,
  ...
  */
  ...
  int ret = connect(socketfd, (struct sockaddr *)&server_address,
                    sizeof(server_address));

  if (ret < 0) {
    cerr << "basesocket.cc:connects:66: Error connected to server (non-zero "
            "return value)"
         << endl;
    return 1;
  }
...
const char *tcp::basic_socket::getLocalhost() {
...
int tcp::basic_socket::getLocalPort() { return this->port; }

int tcp::basic_socket::sends(char *buffer) {
  ...
  if (bytes < 0) {
    // cerr << "basesocket.cc:sends: Error sending size of buffer to socket" <<
    // endl;
    return -1;
  }
...
int tcp::basic_socket::sendBuf(char *data, int size) {
  // make sure connected
  if (!isConnected()) {
...
char *tcp::basic_socket::reads() {
  ...
  buffer =
      new char[buflen +
               1]; // create a buffer for reading with room for null terminator
...
int tcp::basic_socket::readBuf(char *buffer, int size) {
...
int tcp::basic_socket::setOpt(int level, int option, const void *val) {
  socklen_t size = sizeof(val);
  return setsockopt(this->socketfd, level, option, val, size);
}

int tcp::basic_socket::getOpt(int level, int option, void *val) {
  socklen_t size = sizeof(val);
  return getsockopt(this->socketfd, level, option, val, &size);
}

int tcp::basic_socket::setReuseAddr(bool var) {
  int _val = (int)var;
  return this->setOpt(SOL_SOCKET, SO_REUSEADDR, (void *)&_val);
}

int tcp::basic_socket::isReuseAddr() {
  int val = 0;
...
int tcp::basic_socket::setKeepAlive(bool var) {
  int _var = (int)var;
  return this->setOpt(SOL_SOCKET, SO_KEEPALIVE, (void *)&_var);
}

int tcp::basic_socket::isKeepAlive() {
  int val = 0;
...
char *tcp::basic_socket::getPeerName() {
  ...
  if (info.ss_family == AF_INET) {
    ...
    // cerr << "basesocket.cc:301: getPeerName does not support AF_INET6" <<
    // endl;
    return NULL;
  }
...
void tcp::basic_socket::closes() { close(socketfd); }
